...
tweets = []
flag = True
with open('sample_tweets.csv') as f:
    r = csv.reader(f)
    for row in r:
        tweets.append(row[0])
...
# print(tweets)
model = EmotionPredictor(classification='plutchik', setting='ml', use_unison_model=True)
# tweets = [
...
print(probabilities['Tweet'])
print(probabilities['Tweet'])
probabilities.to_csv("output.csv", sep='\t')
